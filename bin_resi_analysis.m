...
% sum(bin_size_real)
% length(OC_range_rate)
% bin_rms_mean=mean(bin_rms);
% 
% cut_no=0;
% for ii=1:length(bin_rms)
%     if bin_rms(ii)>bin_rms_mean
%         ii
%         bin_rms(ii)
%         cut_no=cut_no+1;
%     end
...
% figure
% plot(bin_upper_num,bin_rms,'*b','LineWidth',3,'MarkerSize',3)
% title([' Year ', year,' Day ',ds])
% xlabel('bin number');
...
% bin_sum=0;
% bin_counter=0; 
% %for  ii=bin_size*bin_lower_num(bin_id)+1:bin_size*bin_upper_num(bin_id)
% for  ii=1:length(OC_range_rate)
%     if (bin_lower(bin_id) <= frac(ii) & frac(ii)< bin_upper(bin_id))
%         bin_sum=bin_sum+(OC_range_rate(ii))^2;
%         bin_counter=bin_counter+1;
